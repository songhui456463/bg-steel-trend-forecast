...
from forecasting.modeling_lstm import LSTMSingle, LSTMMultiple
...
def get_gru_hash(gru_model_object):
    """
    根据Lstm_single的参数结构生成唯一的hash
    :param gru_model_object: lstm_single或lstm_multiple的object
    :return:
    """
    params_dict = gru_model_object.__dict__
...
def get_gru_model_path(
    ...
    根据当前训练数据和模型类生成模型文件名
    :param price_history_df: 用来训练模型的序列df
    :param model_object: gru_single或gru_multiple的object，用于生成模型结构的hash
    :return:
    """
    # 形如：outputs \gru \price_name \xxx.pt

    save_base_path = os.path.join(settings.BASE_PATH, "outputs")
    gru_dir_path = os.path.join(save_base_path, "gru")

    # 按价格序列的名称创建一个文件夹
    price_name = price_history_df.columns[0]
    legal_price_name = sanitize_filename(price_name)
    price_dir_path = os.path.join(gru_dir_path, legal_price_name)
    if not os.path.exists(price_dir_path):
        os.makedirs(price_dir_path)

    # 按训练数据创建model文件的名字
    model_hash = get_gru_hash(model_object)
    cur_model_name = (
        f'{model_hash}__{price_history_df.index[0].strftime("%Y%m%d")}-{price_history_df.index[-1].strftime("%Y%m%d")}'
        + ".pth"
    )
