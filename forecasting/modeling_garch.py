...
def arch_modeling(train_df: pd.DataFrame, pre_steps: int = 1):
    ...
    is_autocorr = autocorr_test(resid).get(
        EnumPretestingReturn.autocorrTest_is_corr
    )  # 自相关性
    is_normal = gaussian_test(resid).get(
    ...
    if not is_autocorr and is_normal:
        ...
        mylog.warning(
            f"<{y_name}> GARCH-model residuals 不是白噪声. 理论上GARCH建模失败"
        )
...
def arch_forecasting(
    ...
    # 2 检验价格/收益率序列：平稳性、异方差性 (GARCH模型的输入需要是平稳的和异方差的)
    # 检验平稳性
    res_stationary = stationary_test(df.loc[:, ["diff_0"]])
    is_stationary = res_stationary.get(
        EnumPretestingReturn.stationaryTest_is_stationary
    )
    stationary_d = res_stationary.get(
        EnumPretestingReturn.stationaryTest_stationary_d
    )
    if not is_stationary:  # 若不平稳,需要差分使序列平稳
    ...
    # 平稳后检验异方差性
    is_het = hetero_test(garch_input_df).get(
        EnumPretestingReturn.hetetoTest_is_het
    )
    if not is_het:
        mylog.warning(
